package com.rtllabs.movies.ui.data.network

import retrofit2.Response
import com.rtllabs.movies.ui.data.model.Result
import com.rtllabs.movies.ui.data.network.exception.NoInternetException
import com.rtllabs.movies.ui.data.network.exception.NotFoundException
import com.rtllabs.movies.ui.data.network.exception.UnAuthorizedException
import com.rtllabs.movies.ui.data.network.exception.UnKnownException
import retrofit2.HttpException
import java.lang.Exception
import java.net.HttpURLConnection
import java.net.SocketTimeoutException
import java.net.UnknownHostException


abstract class BaseService {
    /*protected suspend fun <T: Any> makeApiCall(call :suspend ()-> Response<T> ): Result<T> {
        val response: Response<T>
        response=call.invoke()
        if (response.body() != null){
            return Result.Success(response.body()!!)
        }

    }*/
    protected suspend fun<T: Any> createCall(call: suspend () -> Response<T>) : Result<T> {

        val response: Response<T>
        try {
            response = call.invoke()
        }catch (t: Throwable){
            t.printStackTrace()
            return Result.Error(mapToNetworkError(t))
        }

        if (response.isSuccessful){
            if (response.body() != null){
                return Result.Success(response.body()!!)
            }
        }
        else{
            val errorBody = response.errorBody()
            return if (errorBody != null){
                Result.Error(mapApiException(response.code()))
            } else Result.Error(mapApiException(0))
        }
        return Result.Error(HttpException(response))
    }

     fun mapApiException(code: Int): Exception{
         return when(code){
             HttpURLConnection.HTTP_NOT_FOUND ->NotFoundException()
             HttpURLConnection.HTTP_UNAUTHORIZED->UnAuthorizedException()
             else ->UnKnownException()
         }
     }

    private fun mapToNetworkError(t: Throwable): Exception{
         return when(t){
             is SocketTimeoutException->SocketTimeoutException(("Connection Time out"))
             is UnknownHostException -> NoInternetException()
             else -> UnKnownException()
         }
    }

}